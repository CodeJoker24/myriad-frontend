    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto pb-20 md:pb-6">
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2">
          <select 
            className="px-4 py-3 bg-white rounded-xl border border-gray-200 text-sm outline-none focus:ring-2 focus:ring-primary/20"
            value={filterStatus}
                ...
                filteredTeachers.map((t) => (
                  <tr key={t.id} className={`hover:bg-gray-50 transition-colors ${!t.is_active ? 'bg-gray-50/50 opacity-60' : ''}`}>
                    <td className="p-4">
                      ...
                      <div className="text-xs text-gray-400 font-medium tracking-tight">{t.email}</div>
                     </td>
                    <td className="p-4">
                      {t.is_class_teacher_of ? (
                        <div className="flex items-center gap-2 text-purple-600 bg-purple-50 px-2 py-1 rounded-lg w-fit border border-purple-100">
                          <FaChalkboardTeacher size={12} />
                          <span className="text-[10px] font-bold uppercase">Form Teacher: {t.is_class_teacher_of}</span>
                        </div>
                      ) : (
                        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Subject Specialist</span>
                      )}
                     </td>
                    <td className="p-4">
                      <div className="flex flex-wrap gap-1 max-w-50">
                        ...
                        {t.subjects?.length > 3 && <span className="text-[10px] text-gray-400 font-bold">+{t.subjects.length - 3}</span>}
                      </div>
                     </td>
                    <td className="p-4">
                      <span className={`px-2 py-1 rounded-md text-[10px] font-black tracking-tighter ${t.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {t.is_active ? 'ACTIVE' : 'INACTIVE'}
                      </span>
                     </td>
                    <td className="p-4 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => toggleStatus(t.id, t.is_active, t.name)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors text-gray-500 hover:text-orange-500">
                          {t.is_active ? <FaLock size={14} /> : <FaUnlock size={14} />}
                        </button>
                        <button onClick={() => handleEditClick(t)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors text-blue-500">
                          <FaEdit size={14} />
                        </button>
                        <button onClick={() => handleDelete(t.id, t.name)} className="p-2 hover:bg-gray-100 rounded-lg transition-colors text-red-500">
                          <FaTrash size={14} />
                        </button>
                      </div>
                     </td>
                ...
                <div className="px-4 pb-4 pt-2 border-t border-gray-100 space-y-3">
                  ...
                  <div className="bg-purple-50 rounded-xl p-3">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs font-bold text-purple-600 uppercase">Designation</span>
                    </div>
                    {t.is_class_teacher_of ? (
                  ...
                  <div className="flex gap-3 pt-3">
                    ...
                    <button 
                      ...
                      className="flex-1 py-2.5 rounded-xl text-sm font-medium bg-blue-50 text-blue-600 active:bg-blue-100 transition-colors"
                    >
                      <FaEdit size={12} className="inline mr-1" /> Edit
                    </button>
                    <button 
                      onClick={() => handleDelete(t.id, t.name)} 
                      className="flex-1 py-2.5 rounded-xl text-sm font-medium bg-red-50 text-red-600 active:bg-red-100 transition-colors"
                    >
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowAddModal(false)}>
          <div className="bg-white rounded-t-3xl md:rounded-3xl w-full max-w-2xl mx-auto p-5 md:p-8 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-xl md:text-2xl font-bold text-gray-900 tracking-tight">{editingId ? 'Edit Profile' : 'New Teacher Account'}</h2>
            ...
            <form onSubmit={handleSave} className="space-y-5">
              ...
              <button disabled={loading} className="w-full bg-primary hover:bg-primary-dark text-white py-3.5 rounded-xl font-bold mt-6 shadow-md transition-all active:scale-[0.98] flex items-center justify-center gap-2 text-sm">
