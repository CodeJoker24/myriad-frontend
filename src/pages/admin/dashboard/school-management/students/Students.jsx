      ...
      if (editingId) {
        ...
        if (authErr) {
          if (authErr.message.includes("already registered")) {
            throw new Error(`This Student ID (${nextId}) already has an account.`);
          }
          throw authErr;
        }
        ...
        if (dbErr) {
          throw new Error(`Account created, but profile failed: ${dbErr.message}`);
        }
        ...
        Swal.fire({
          ...
          html: `
            <div class="text-left bg-gray-50 p-4 rounded-xl mt-2">
              <p class="text-sm"><strong>Student ID:</strong> <code class="text-primary font-bold">${nextId}</code></p>
              <p class="text-sm"><strong>Login Email:</strong> <code>${internalEmail}</code></p>
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto">
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none text-sm"
              />
            </div>
            <button
              onClick={() => setShowFilters(!showFilters)}
              className={`px-4 py-3 rounded-xl flex items-center gap-2 transition-all ${showFilters ? 'bg-primary text-white' : 'bg-gray-50 text-gray-600 border border-gray-200'}`}
            >
              <FaFilter size={14} />
          ...
          {showFilters && (
            <div className="mt-4 pt-4 border-t border-gray-100">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <select 
                  className="px-4 py-2.5 bg-gray-50 rounded-xl border border-gray-200 text-sm outline-none focus:ring-2 focus:ring-primary/20"
                  value={filterStatus}
                      ...
                      <tr key={s.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="py-3 px-5">
                          <div className="flex items-center justify-center gap-1.5">
                            <button onClick={() => handleQuickView(s)} className="p-2 text-indigo-500 hover:bg-indigo-50 rounded-lg transition-colors" title="Quick View">
                              <FaEye size={14} />
                            </button>
                            <button onClick={() => toggleStatus(s.id, s.is_active, s.name)} className="p-2 text-orange-500 hover:bg-orange-50 rounded-lg transition-colors" title="Suspend/Activate">
                              <FaBan size={14} />
                            </button>
                            <button onClick={() => { setEditingId(s.id); setFormData(s); setShowAddModal(true); }} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                              <FaEdit size={14} />
                            </button>
                            <button onClick={() => handleDelete(s.id, s.name)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
                              <FaTrash size={14} />
                            </button>
                          </div>
                        </td>
        ...
        {showViewModal && viewingStudent && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setShowViewModal(false)}>
            <div className="bg-white rounded-2xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
              ...
              <button onClick={() => setShowViewModal(false)} className="w-full bg-primary text-white py-3 rounded-xl font-semibold mt-4">
                Close
              </button>
            </div>
          </div>
        ...
        {showAddModal && (
          <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowAddModal(false)}>
            <div className="bg-white rounded-t-2xl md:rounded-2xl w-full max-w-2xl mx-auto p-5 shadow-2xl max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
